...
std::atomic_bool is_thread_stoped[thread_count] = {false};
    ...
    while (!thread_end)
    {
        ...
        for (int j = start; j < end; j++)
        {
            ...
            else
            {
                ...
                {
                    // m.lock();
                    u_x_t = tau * ((a.prev_str[j - 1] - 2 * a.prev_str[j] + a.prev_str[j + 1]) / pow(h, 2)) +
                            a.prev_str[j];
                    a.current_str[j] = u_x_t;
                    // std::cout << u_x_t << ' ';
                    // m.unlock();
...
{
    arrs a;
    // cout << std::fixed << std::setprecision(6); // Задаем точность для double в кауте, думаю будет
    // достаточно
    ...
    for (int thrd = 0; thrd < thread_count; ++thrd)
    {
        ...
        {
            thread_arr[thrd] = std::thread(in_Thread, mlt_for_loop[thrd], n_l_int, &a, thrd);
            // ---1---
        ...
        {
            thread_arr[thrd] = std::thread(in_Thread, mlt_for_loop[thrd], mlt_for_loop[thrd + 1], &a, thrd);
            // ---1--- ver of realization
            /*thread_arr[thrd] = std::thread([&]() {
            ...
            // ---vec--- thread_arr.push_back(thrd_to_push); // ---vec--- ver
        }
    }
    ...
    std::cout << "Time difference = "
              << std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count() / 1'000'000.0
              << "[ms] " << std::endl;
